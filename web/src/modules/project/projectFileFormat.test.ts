...
describe("project file format and model", () => {
  ...
  it("rejects malformed, wrong-module, unsupported-version, duplicate-name, and invalid-record files", () => {
    ...
    expect(() => parseProjectFileJson(JSON.stringify({ module: "wrong", schemaVersion: 1, projects: [] }))).toThrow(
      "missing or invalid project data"
    );
    expect(() =>
      parseProjectFileJson(JSON.stringify({ module: "project-management", schemaVersion: 999, projects: [] }))
