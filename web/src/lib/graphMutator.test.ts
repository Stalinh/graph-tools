...
describe("graphMutator", () => {
  ...
  it("removes bidirectional citations from both sides", () => {
    const linked = addCitation(graph([cardNode("#1", "One"), cardNode("#2", "Two")]), "#1", "#2", "bidirectional");
  ...
  it("removes and restores a node with edges and affected references", () => {
    ...
    expect(restoreNode(removed.graph, removed.removedNode!, removed.removedEdges, removed.affectedRefs)).toEqual(
      initial
    );
  });
  ...
  it("keeps references consistent when edge direction and order change", () => {
    const linked = addCitation(graph([cardNode("#1", "One"), cardNode("#2", "Two"), cardNode("#3", "Three")]), "#1", "#2");
